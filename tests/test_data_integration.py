...
class TestFetchFreshData:
    ...
    def test_fetch_timeout(self):
        ...
        with patch('app.subprocess.run', side_effect=subprocess.TimeoutExpired("node", 30)):
            result = fetch_fresh_data()
    ...
    def test_fetch_file_not_found(self):
        ...
        with patch('app.subprocess.run', side_effect=FileNotFoundError("node command not found")):
            result = fetch_fresh_data()
    ...
    def test_fetch_with_output_logging(self):
        ...
        with patch('app.subprocess.run', return_value=mock_result), \
            ...
            mock_logger.info.assert_called_with("Successfully fetched fresh departure data")
            mock_logger.debug.assert_called_with(f"Node.js output: {mock_result.stdout}")
...
class TestUpdateData:
    ...
    def test_successful_update(self):
        ...
        with patch('app.fetch_fresh_data', return_value=True) as mock_fetch, \
            ...
            mock_logger.info.assert_called_with("Successfully fetched fresh data, reloading")

    def test_failed_update(self):
        """Test failed data update"""
        with patch('app.fetch_fresh_data', return_value=False) as mock_fetch, \
             patch('app.st.rerun') as mock_rerun, \
             patch('app.logger') as mock_logger:
            ...
            mock_logger.warning.assert_called_with("Failed to fetch fresh data, using cached data")
...
class TestDataPipelineIntegration:
    ...
    def test_pipeline_with_fetch_and_reload(self, sample_train_data: dict[str, Any]):
        ...
        with patch('app.subprocess.run', return_value=mock_subprocess_result), \
    ...
    def test_pipeline_error_recovery(self):
        ...
        cached_data = {"badVoeslauToWien": [], "wienToBadVoeslau": [], "lastUpdated": ""}
...
class TestRealTimeDataProcessing:
    ...
    def test_realtime_delay_processing(self):
        ...
        with patch('app.os.path.exists', return_value=True), \
            ...
            delay2 = calculate_delay(trains[1]["ti"], trains[1].get("rt", {}).get("dlt"))
            assert delay2 == 0
